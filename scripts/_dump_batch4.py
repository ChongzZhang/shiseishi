...
start = next(i for i, c in enumerate(ordered) if c["pinyin"] == "yunfengbai")
out = []
...
Path(__file__).with_name("_review_batch4.txt").write_text("\n".join(out), encoding="utf-8")
print("batch4 from idx", start, "nonempty", len([x for x in out if x.startswith("\n===")]))
